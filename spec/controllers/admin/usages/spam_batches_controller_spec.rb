RSpec.describe Admin::Usages::SpamBatchesController, type: :controller do
  ...
  describe "POST #create" do
    ...
    let!(:usage) { create(:usage, created_at: time) }
    let(:time) { 1.hour.ago }

    before do
      # 変更されない
      create(:usage, status: :approved, created_at: time - 1.second)
      create(:usage, status: :unconfirmed, created_at: time + 1.second)
    end

    context "with authority" do
      let(:authority) { "admin" }
      it "指定日時より古い未確認をスパムにして一覧に戻すこと" do
        expect_any_instance_of(Card::Usage).to receive(:mark_spam!).exactly(1).times
  ...
  describe "POST #create プロジェクト単位の絞り込み安全性" do
    ...
    context "project_id を指定したとき" do
      ...
      it "スパム化されるのは対象2件だけで他へ波及しないこと" do
        expect { subject }.to change { Card::Usage.spam.count }.from(0).to(2)
      end
    end
    ...
    def mk(project, status, created_at)
      create(:usage, project: project, status: status, created_at: created_at)
    end
  end
end
