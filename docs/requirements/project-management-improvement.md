...
**検出時の動作**
- 投稿を拒否し、禁止キーワードが含まれている旨をエラーメッセージで表示する
- エラーメッセージには検出されたキーワードを伏字付きで表示する（例：「c\*\*\*\*o」）
  - 4文字以上のキーワード：先頭1文字と末尾1文字を表示し、中間を「\*」で伏せる
  - 3文字以下のキーワード：伏字なしで「禁止キーワードが含まれています」とだけ表示
...
| スパムキーワード | 投稿内容に含まれる場合に投稿を拒否するキーワード |
| 伏字表示 | キーワードの一部を「*」で隠して表示する方式（例：casino → c\*\*\*\*o） |
...
### スパムキーワード検出
- [ ] キーワードを含むプロジェクト作成がエラーメッセージ付きで拒否されること
- [ ] キーワードを含むプロジェクト更新がエラーメッセージ付きで拒否されること
- [ ] キーワードを含むコメント投稿がエラーメッセージ付きで拒否されること
- [ ] 4文字以上のキーワードは伏字付きで表示されること（例：c\*\*\*\*o）
- [ ] 3文字以下のキーワードは伏字なしで「禁止キーワードが含まれています」と表示されること
