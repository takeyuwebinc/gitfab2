...
- position が同じカードの表示順が id の昇順に固定される。対象はプロジェクト詳細、スライドショー、カード一覧、バックアップである。現在の表示順が id の昇順と違うカードがあると、その表示順が変わる。本番の読み取り調査（2026-09-21）では、position が重複している範囲（State 33 件、Annotation 239 件）のうち 4 範囲の 9 枚が該当した（State が 2 範囲、Annotation が 2 範囲）。いずれも同じ position の 2〜3 枚が入れ替わる
...
| ID | 変更内容の項目 | 種類 | 条件 |
|----|--------------|------|------|
| AC-1 | カードの並びを確定する責務 | 正常 | position が連番のカードを 3〜5 枚持つ範囲で、全順列のどの並びを渡しても、確定後に position、id の昇順で読んだ並びが指定と一致し、position が 1 からの連番になる。id と position の組は、配列でも、index をキーにした Hash（フォームが送る nested attributes の形）でも受け付ける |
| AC-2 | 同上 | 正常 | position が変わったカードだけ `updated_at` が更新され、position が変わらないカードの `updated_at` は変わらない |
| AC-3 | 同上 | 異常・拒否 | 範囲に属さない id を含む場合は「見つからない」、position が空文字、nil、小数、数字以外の場合と、id か position を持たない組を含む場合と、同じ id を複数回含む場合は「不正なリクエスト」として拒否され、どのカードの position も `updated_at` も変わらない |
...
| AC-10 | 同上 | 境界 | `states_attributes` が空のリクエストと、`project` キーを含まないリクエストは成功が返り、State の position は変わらず、プロジェクトの `updated_at` は更新される |
...
| AC-22 | State の並べ替え | 異常・拒否 | プロジェクトが検証に通らない場合は 400 と `{ success: false }` が返り、State の position とプロジェクトの `updated_at` は変わらない（並べ替えの書き込みも取り消される） |
| AC-23 | Annotation の並べ替え | 境界 | `annotations_attributes` のうち position を持たない組は拒否せず、並べ替えの依頼に含めない（AC-7 の「リクエストに含まれないカード」として扱う）。position を持つ組が 1 つもなければ並びを確定せず、position の重複や欠番もそのまま残る |
| AC-24 | 同上 | 正常 | 並べ替えと同時に送った Annotation の position 以外の属性（title など）は、現状どおり保存される |
| AC-25 | 同上 | 異常・拒否 | State の保存の後に並びの確定で例外が起きると、State の属性と contributor の追加も取り消され、更新通知は送られない |
...
次の分岐は実装にあるが、画面から届かない、または形の違いだけであるため、受け入れ条件に行を設けない。

- id と position の組が Hash でない依頼は「不正なリクエスト」として拒否する。`states_attributes` と `annotations_attributes` が index をキーにしない単一の Hash（`[id]=1&[position]=2`）で届いた場合も同じく 400 になる。Strong Parameters が配列中のスカラーを落とすため、それ以外の形は HTTP からは届かない。画面は常に index をキーにした Hash を送る
- id は文字列のまま範囲の id と比べるため、`"01"` のような表記は「見つからない」になる。画面の JavaScript は DOM の id 属性をそのまま送る
- `annotations_attributes` が配列で届いても、index をキーにした Hash と同じに扱う。形の違いはモデルの受け付ける形（AC-1）と同じ

### 未解決の疑問

- なし。表示順に第 2 キーを加えると現在の表示順が変わるカードは、本番に 4 範囲 9 枚あった（影響範囲を参照）。同じ position の行の順序は MySQL が保証せず、実行計画で決まる。同じプロジェクトの中でも、position によって id の降順と昇順が混在していた。守るべき順序がないため、第 2 キーを入れる（2026-09-21 決定）
