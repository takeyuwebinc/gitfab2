...
| 2.5 | スパマー登録の自動解除 | **ChangeSpec作成済（未実装）** | `SpamMarkable#unmark_spam!` は status 戻しのみで Spammer 解除フックなし。2.4 で `User#spam_undetect!`（Spammer 破棄）を新設済み。ChangeSpec: [spammer-auto-unregistration.md](../change-specs/spammer-auto-unregistration.md)。唯一の実装変更は `unmark_spam!` への `spam_author&.spam_undetect!` 追加（プロジェクト取消経路は 2.4 で充足） |
...
| 4 | 2.5 スパマー登録の自動解除 | 2.1, 2.4 | `SpamMarkable#unmark_spam!` への Spammer 自動解除追加（コメント/Usage/Annotation/Tag を単一 concern で網羅。プロジェクト取消は 2.4 で充足） | **ChangeSpec作成済**（[spammer-auto-unregistration.md](../change-specs/spammer-auto-unregistration.md)）。次は development-start で実装 |
