...
| 2.5 | スパマー登録の自動解除 | **実装済**（01d080d0） | `SpamMarkable#unmark_spam!` を `mark_spam!` の対操作にし、`spam_author&.spam_undetect!` で Spammer 自動解除。コメント/Card::Usage/Card::Annotation/Tag の全解除経路を単一 concern で網羅。プロジェクト取消経路は 2.4 で充足済み。spec は同コミット。ChangeSpec 未廃棄（2.3/2.4 と一括クリーンアップ対象） |
...
| 4 | 2.5 スパマー登録の自動解除 | 2.1, 2.4 | `SpamMarkable#unmark_spam!` への Spammer 自動解除追加（コメント/Usage/Annotation/Tag を単一 concern で網羅。プロジェクト取消は 2.4 で充足） | **完了**。実装 01d080d0（spec 同コミット）。ChangeSpec 未廃棄（2.3/2.4 と一括クリーンアップ対象） |
...
- **進捗同期（2026-06-08 / 2.5 実装完了）**: 2.5「スパマー登録の自動解除」を実装（01d080d0）。`SpamMarkable#unmark_spam!` を `mark_spam!` の対操作とし、`with_lock` 内で `spam_author&.spam_undetect!` を呼ぶ。TDD（3 spec に Spammer 解除・nil ガード・冪等を追加）→ green、spam 系回帰 190 examples 0 failures。**これで要件 2.1〜2.5 がすべて実装完了**。残作業はクリーンアップのみ：2.3/2.4/2.5 の ChangeSpec＋本進捗管理ファイルを doc-retirement で Why 蒸留してから一括廃棄する（設計コンテキスト保持のため実装完了後に実施）。
- **進捗同期（2026-06-08 / 2.5 ChangeSpec 作成）**: 推奨順 4 の 2.5「スパマー登録の自動解除」の ChangeSpec を作成（[spammer-auto-unregistration.md](../change-specs/spammer-auto-unregistration.md)）。コード精査の結論として、残実装は `SpamMarkable#unmark_spam!` への `spam_author&.spam_undetect!` 追加（`mark_spam!` の対称・全 markable 経路を単一 concern で網羅）のみ。プロジェクト取消経路は 2.4 で充足済み（受け入れ確認のみ）。Phase 3 自動品質検証で設計の誤りなし（記述の正確性4点を修正）。
- **進捗同期（2026-06-08 / 2.3・2.4 完了）**: 2.3（非破壊化＋通常削除と区別する `spam_hidden_at` カラム追加）と 2.4（取消機能：`unhide_as_spam!`・取消サービス・Admin UI、認定済み一覧はキーワード検索可）を実装完了。2.3/2.4 の ChangeSpec は結合ペアとして 2.5 と一括クリーンアップ予定のため未廃棄。
